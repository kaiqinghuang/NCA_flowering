...
class KinectDepthSource:
    ...
    # ---------- public API ----------
    def start(self) -> None:
        # Idempotent: safe to call from multiple lifespan paths (e.g.
        # the parent app forwards the bridge sub-app's @on_event
        # handlers in pclocal/app.py).
    ...
    def _do_auto_calibration(self, runtime) -> None:
        ...
        try:
            ...
            try:
                ...
                tad_env = os.environ.get("BRIDGE_AUTOFIT_TRIM_AD_PCT", "0.5")
                tbc_env = os.environ.get("BRIDGE_AUTOFIT_TRIM_BC_PCT", "0.5")
